 ...
 *
 * <p>Runs at {@code @Order(10)} — after seeders ({@code @Order(1)}) and
 * {@code HmacKeyBootstrapService} ({@code @Order(5)}) which ensures HMAC rows exist
...
public class KeyRingInitialiser implements ApplicationRunner {
    ...
    private void loadHmacRing() {
        ...
        if (hmacVersions.isEmpty()) {
            log.warn("KeyRingInitialiser: no ACTIVE or ROTATING HMAC key found in key_versions. "
                    + "Tokenisation will fail until an HMAC key is seeded via HmacKeyBootstrapService.");
            return;
        }
    ...
    private void loadHmacVersion(KeyVersion hv) {
        // Look up the KEK that was used to encrypt this HMAC secret
        String encryptingKekId = hv.getEncryptingKekId() != null
                ? hv.getEncryptingKekId().toString()
                : keyVersionRepository.findActiveKekOrThrow().getId().toString();

        KeyMaterial kekMaterial = keyRing.getByVersion(encryptingKekId);
        byte[] kek = kekMaterial.copyKek();
        byte[] secret = null;
        try {
            secret = cipher.decryptBytes(hv.getEncryptedSecret(), kek);
